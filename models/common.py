...
from utils.geometry import estimate_pointcloud_normals_from_two_pcds


class PointNetSetAbstractionRatio(nn.Module):
    def __init__(self, ratio, radius, nsample, in_channel, mlp, group_all,
    ...
    def forward(self, xyz, points, fps_idx=None):
        ...
        if (self.group_all == False) and (npoint != -1):
            if fps_idx == None:
...
class PointNetFeaturePropogation(nn.Module):
    def __init__(self, in_channel, mlp, learn_mask=False, nsample=3):
        ...
        if learn_mask:
            self.queryandgroup = pointutils.QueryAndGroup(None, 9, use_xyz=True)
            last_channel = (128 + 3)
    ...
    def forward(self, pos1, pos2, feature1, feature2, hidden=None):
        ...
        if hidden is None:
            ...
            interpolated_feat = torch.sum(pointutils.grouping_operation(feature2, idx) * weight.view(B, 1, N, self.nsample),
                                        dim=-1)  # [B,C,N,3]
        else:
            dist, idx = pointutils.knn(9, pos1_t, pos2_t)

            new_feat, _ = self.queryandgroup(pos2_t, pos1_t, hidden)  # [B, 3+C, N, 9]

            for i, conv in enumerate(self.mlp_convs):
                new_feat = conv(new_feat)
            weight = torch.softmax(new_feat, dim=-1)  #[B, 1, N, 9]
            interpolated_feat = torch.sum(pointutils.grouping_operation(feature2, idx) * weight,
                                          dim=-1)  # [B, C, N]   
...
class Sinkhorn(nn.Module):
    ...
    def forward(self, corr, epsilon, gamma, max_iter):
        ...
        a = (
            torch.ones(
                (corr.shape[0], corr.shape[1], 1), device=corr.device, dtype=corr.dtype
            )
            / corr.shape[1]
        )
        prob1 = (
        ...
        prob2 = (
            torch.ones(
                (corr.shape[0], corr.shape[2], 1), device=corr.device, dtype=corr.dtype
            )
            / corr.shape[2]
        )
...
class PointWiseOptimLayer(nn.Module):
    def __init__(self, nsample, radius, in_channel, mlp, use_curvature=True):
        ...
        self.pos_embed = nn.Sequential(nn.Conv1d(3, 32, 1), nn.ReLU(inplace=True), nn.Conv1d(32, 64, 1))

        self.qk_net = nn.Sequential(nn.Conv1d(in_channel + 64, in_channel + 64, 1), nn.ReLU(inplace=True),
                                    nn.Conv1d(in_channel + 64, in_channel + 64, 1))
        if self.use_curvature:
            self.curvate_net = nn.Sequential(nn.Conv1d(3, 32, 1), nn.ReLU(inplace=True),
                                             nn.Conv1d(32, 32, 1))
            self.mlp_conv = nn.Conv1d(in_channel + 64 + 32, mlp[-1], 1, bias=True)
        else:
            self.mlp_conv = nn.Conv1d(in_channel + 64, mlp[-1], 1, bias=True)     
                
    def forward(self, pos1, pos2, feature1, feature2, nsample, radius=None, pos1_raw=None, return_score=False):
        """
        Input:
        ...
        feat2_w_pos = torch.cat([feature2, pos2_embed_norm], dim=1) # [B, C1+C2, N]
        
        feat1_w_pos = self.qk_net(feat1_w_pos)
        feat2_w_pos = self.qk_net(feat2_w_pos) # [B, C1+C2, N]

        feat2_grouped = pointutils.grouping_operation(feat2_w_pos, idx)  # [B, C1+C2, N, S]

        score = torch.softmax(feat1_w_pos.unsqueeze(-1) * feat2_grouped * 1./ math.sqrt(feat1_w_pos.shape[1]), dim=-1) # [B, C1+C2, N, S]
        cost = (score * (feat1_w_pos.unsqueeze(-1) - feat2_grouped) ** 2).sum(dim=-1) # [B, C1+C2, N]

        if self.use_curvature:
            curvate1_raw = curvature(pos1_raw).permute(0, 2, 1) #[B, 3, N]
            curvate1 = curvature(pos1).permute(0, 2, 1) #[B, 3, N]
            curvate_cost = self.curvate_net(curvate1_raw) - self.curvate_net(curvate1)
            curvate_cost = curvate_cost ** 2
            cost = self.mlp_conv(torch.cat([cost, curvate_cost], dim=1)) # [B, C, N]
        else:
        ...
        if return_score:
            ...
            # [B, N, K]
            index = (dist > self.radius).sum(dim=2, keepdim=True).float() > (dist.shape[2] - 0.1) #[B, N, 1]
            index = index.unsqueeze(1).repeat(1, score.shape[1], 1, dist.shape[2]) #[B, N, K]
