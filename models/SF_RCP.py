...
from models.common import PointNetSetAbstraction, PointNetFeaturePropogation, PointWiseOptimLayer, Sinkhorn
...
class FeatureMatching(nn.Module):
    ...
    def upsample_flow(self, pc1_l, pc1_l_glob, flow_inp):
        ...
        flow_up, flow_lr_up = self.flow_up_sample(pc1_l["s1"], pc1_l_glob["s32"], None, flow_inp), \
                              self.flow_up_sample(pc1_l["s4"], pc1_l_glob["s32"], None, flow_inp)
    ...
    def calc_feats_corr(self, pcloud1, pcloud2, feature1, feature2, norm):
        ...
        """
        if norm:
            feature1 = feature1 / torch.sqrt(torch.sum(feature1 ** 2, -1, keepdim=True) + 1e-6)
            feature2 = feature2 / torch.sqrt(torch.sum(feature2 ** 2, -1, keepdim=True) + 1e-6)
            corr_mat = torch.bmm(feature1, feature2.transpose(1, 2)) # [B, N1, N2]
        else:
            corr_mat = torch.bmm(feature1, feature2.transpose(1, 2)) / feature1.shape[2]**.5 #[B, N1, N2]

        if self.support_th is not None:
            distance_matrix = torch.sum(pcloud1 ** 2, -1, keepdim=True) #[B, N1, 1]
            distance_matrix = distance_matrix + torch.sum(
                pcloud2 ** 2, -1, keepdim=True
            ).transpose(1, 2)  #[B, N1, N2]
            distance_matrix = distance_matrix - 2 * torch.bmm(pcloud1, pcloud2.transpose(1, 2))  #[B, N1, N2]
            support = (distance_matrix < self.support_th).float()   #[B, N1, N2]
        else:
    ...
    def get_flow_init(self, pcloud1, pcloud2, feats1, feats2):
        ...
        flow_init = (corr_mat @ pcloud2.permute(0, 2, 1).contiguous()) / (row_sum + 1e-6) - pcloud1.permute(0, 2, 1).contiguous()  #[B, N1, 3]
...
class FlowRegressor(nn.Module):
    def __init__(self, npoint, use_instance_norm, input_dim=128, nsample=32):
        ...
        self.sa1 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=nsample, in_channel=input_dim, mlp=[input_dim, input_dim],
                                          group_all=False, use_instance_norm=use_instance_norm)
        self.sa2 = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=nsample, in_channel=input_dim, mlp=[input_dim, input_dim],
                                          group_all=False, use_instance_norm=use_instance_norm)

        self.fc = nn.Sequential(nn.Linear(input_dim, input_dim), nn.ReLU(inplace=True),
    ...
    def forward(self, pc1_l, feats):
        ...
        flow = self.up_sample(pc1_l["s1"], pc1_l["s4"], None, flow_lr) #[B, 3, N]
...
class GRUReg(nn.Module):
    def __init__(self, npoint, hidden_dim, input_dim, use_instance_norm):
        ...
        self.flow_proj = nn.ModuleList(
                                [PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=16, in_channel=3, mlp=[32, 32, 32],
                                                        group_all=False, use_instance_norm=use_instance_norm),
                                 PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=8, in_channel=32, mlp=[16, 16, 16],
                                                        group_all=False, use_instance_norm=use_instance_norm)])

        self.hidden_init_net = HiddenInitNet(npoint, use_instance_norm)

        self.gru_layers = nn.ModuleList([
            PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=in_ch,
    ...
    def get_gru_input(self, feats1_new, cost, flow, pc):
        ...
        gru_inp = torch.cat([feats1_new, cost, flow_feats, flow], dim=1) # [64, 128, 16, 3]
...
class SF_RCP(nn.Module):
    ...
    def pointwise_optimization(self, pc1_l_new, pc2_l, feats1_new, feats2, pc1_l, flow_lr, iter):
        ...
        delta_flow_tmp = ((pos2_grouped - pc1_l_new["s4"].unsqueeze(-1)) * score.mean(dim=1, keepdim=True)).sum(dim=-1) #[B, 3, N]
